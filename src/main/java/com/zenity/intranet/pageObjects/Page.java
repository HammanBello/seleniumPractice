...
import com.zenity.intranet.config.Env;
...
public class Page {
    ...
    public Page() {
        ...
        this.veryShortWait = new WebDriverWait(this.driver, Duration.ofSeconds(2));
    ...
    protected <V>boolean mediumUntil(Function<? super WebDriver, V> isTrue){
        try{
            veryShortWait.until(isTrue);
            return true;
        }catch (Exception e){
            return false;
        }
    ...
    protected void clickOn(WebElement element){
        ...
        if( !longUntil(elementToBeClickable(element))){
